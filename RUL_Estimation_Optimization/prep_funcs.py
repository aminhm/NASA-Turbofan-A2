...
def build_X(df, drop_constant=False, drop_op_setting=False, show=False):
    """
...
def split_dataframes(df, c_perc=0.6, v_perc=0.2, t_perc=0.2):
    '''
    ...
    # split rows by units
    cal_X = df[df['unit_number'].isin(cal_units)].copy()
    val_X = df[df['unit_number'].isin(val_units)].copy()
    test_X = df[df['unit_number'].isin(test_units)].copy()

    # split RUL dataframes by units
    cal_y = cal_X[['unit_number', 'time_in_cycles']].copy()
    val_y = val_X[['unit_number', 'time_in_cycles']].copy()
    test_y = test_X[['unit_number', 'time_in_cycles']].copy()
